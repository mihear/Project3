    ...
    public class FaceBookController : ApiController
    {
        ...
        public IHttpActionResult TopEffectingUser()
        {
            var date = db.Database
               .SqlQuery<OrderRate>("select top (10) Name,COUNT(Id)  as Count from FacebookPosts where Topic='food' group by Name order by Count desc")
               .ToList();
        ...
        {
            var date = db.Database
               .SqlQuery<OrderRate>("select top (10) Resturant as Name,COUNT(Id)  as Count from FacebookPosts where Topic='food' group by Resturant order by Count desc")
               .ToList();
            date.Add(new OrderRate()
            {
                Name = "Design",
        ...
        {
            var date = db.Database
                .SqlQuery<PostActivtity>("select top(10) Post ,(case when IsPositive in(1) then  1 when IsPositive in(0) then  -1 when IsPositive in(2) then  0   end  *  Cast(Postivity as float) *(case when RIGHT([Like], 1) in ('K','k') then CAST(Left([Like], len([Like]) -1)  as float) *1000 " +
                ...
                " else cast(Sad as float)  end)) as Count from FacebookPosts  where Topic='food' order by Count desc")
                .ToList();
            return Ok(date);
        }
        [HttpPost]
        public IHttpActionResult LessActivityPost()
        {
            var date = db.Database
              ...
              " case when RIGHT(Sad, 1) in ('K', 'k') then CAST(Left(Sad, len(Sad) - 1) as float) * 1000 " +
              " when RIGHT(Sad, 1) in ('M', 'm') then CAST(Left(Sad, len(Sad) - 1) as float) * 1000000 " +
              " else cast(Sad as float)  end)) as Count from FacebookPosts  where Topic='food' order by Count")
              .ToList();
            return Ok(date);
        }
        [HttpPost]
        ...
        {
            var date = db.FacebookPosts.Where(m => m.IsPositive == "1" & m.Topic == "food").Count();
            return Ok(date);
        }
        [HttpPost]
        public IHttpActionResult Negative()
        {
            var date = db.FacebookPosts.Where(m => m.IsPositive == "0" & m.Topic == "food").Count();
            return Ok(date);
        }
        [HttpPost]
        public IHttpActionResult Gray()
        {
            var date = db.FacebookPosts.Where(m => m.IsPositive == "2" & m.Topic == "food").Count();
            return Ok(date);
        }
